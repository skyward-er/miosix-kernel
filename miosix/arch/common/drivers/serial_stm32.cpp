...
#endif  //! defined(STM32F411xE) && !defined(STM32F401xE) &&
        //! !defined(STM32F401xC)
...
#else                        // stm32f2 and f4
    DMA2->HIFCR = DMA_HIFCR_CTCIF7 | DMA_HIFCR_CTEIF7 | DMA_HIFCR_CDMEIF7 |
                  DMA_HIFCR_CFEIF7;
#endif
    if (ports[0])
        ports[0]->IRQhandleDMAtx();
}
...
#else                        // stm32f2 and f4
    DMA1->HIFCR = DMA_HIFCR_CTCIF6 | DMA_HIFCR_CTEIF6 | DMA_HIFCR_CDMEIF6 |
                  DMA_HIFCR_CFEIF6;
#endif
    if (ports[1])
        ports[1]->IRQhandleDMAtx();
}
...
#else                        // stm32f2 and f4
    DMA1->LIFCR = DMA_LIFCR_CTCIF3 | DMA_LIFCR_CTEIF3 | DMA_LIFCR_CDMEIF3 |
                  DMA_LIFCR_CFEIF3;
#endif
    if (ports[2])
        ports[2]->IRQhandleDMAtx();
}
...
{
    commonInit(id, baudrate, tx, rx, tx,
               rx);  // The last two args will be ignored
}
                             ...
                             GpioPin rts, GpioPin cts)
{
    ...
    if (id < 1 || id > numPorts || ports[id - 1] != 0)
...
// defined in order to avoid "unused variable" warning
    ...
    defined(_ARCH_CORTEXM4_STM32L4)
            ...
            DMA1_CSELR->CSELR |=
                (2 << DMA_CSELR_C4S_Pos)     // Assign DMA1_CH4 to USART1_TX
            ...
            if (RCC->D2CFGR & RCC_D2CFGR_D2PPRE2_2)
                freq /=
                    1 << (((RCC->D2CFGR >> RCC_D2CFGR_D2PPRE2_Pos) & 0x3) + 1);
    ...
    defined(_ARCH_CORTEXM4_STM32L4)
            ...
            DMA1_CSELR->CSELR |=
                (2 << DMA_CSELR_C7S_Pos)     // Assign DMA1_CH7 to USART2_TX
        ...
        // usart2 clock is rcc_pclk1
            ...
            if (RCC->D2CFGR & RCC_D2CFGR_D2PPRE1_2)
                freq /=
                    1 << (((RCC->D2CFGR >> RCC_D2CFGR_D2PPRE1_Pos) & 0x3) + 1);
#endif  //_ARCH_CORTEXM7_STM32H7
            break;
#if !defined(STM32F411xE) && !defined(STM32F401xE) && !defined(STM32F401xC)
        case 3:
    ...
    defined(_ARCH_CORTEXM4_STM32L4)
            ...
            RCC->AHB1ENR |= RCC_AHB1ENR_DMA1EN;
            DMA1_CSELR->CSELR |=
                (2 << DMA_CSELR_C2S_Pos)     // Assign DMA1_CH2 to USART2_TX
...
// it in ALTERNATE mode. Go figure...
    ...
    const unsigned int quot =
        2 * freq / baudrate;            // 2*freq for round to nearest
    port->BRR = quot / 2 + (quot & 1);  // Round to nearest
    if (flowControl == false)
        port->CR3 |= USART_CR3_ONEBIT;
    else
        port->CR3 |= USART_CR3_ONEBIT | USART_CR3_RTSE | USART_CR3_CTSE;
...
ssize_t STM32Serial::writeBlock(const void *buffer, size_t size, off_t where)
{
    ...
    if (dmaTx)
    {
        ...
        {
            // Use zero copy for all but the last txBufferSize bytes, if
            // possible
            while (remaining > txBufferSize)
            {
                // DMA is limited to 64K
                size_t transferSize =
                    min<size_t>(remaining - txBufferSize, 65535);
                waitDmaTxCompletion();
        ...
        {
            size_t transferSize =
                min(remaining, static_cast<size_t>(txBufferSize));
            waitDmaTxCompletion();
...
#if defined(_ARCH_CORTEXM3_STM32) || defined(_ARCH_CORTEXM4_STM32F3) || \
    defined(_ARCH_CORTEXM4_STM32L4)
        // If no DMA transfer is in progress bit EN is zero. Otherwise wait
        // until DMA xfer ends, by waiting for the TC (or TE) interrupt flag
        static const unsigned int irqMask[] = {(DMA_ISR_TCIF4 | DMA_ISR_TEIF4),
...
#endif
        while ((dmaTx->CCR & DMA_CCR4_EN) &&
               !(DMA1->ISR & irqMask[getId() - 1]))
            ;
#else   //_ARCH_CORTEXM3_STM32
        // Wait until DMA xfer ends. EN bit is cleared by hardware on transfer
        // end
...
int STM32Serial::ioctl(int cmd, void *arg)
{
    ...
    switch (cmd)
    {
        ...
        case IOCTL_TCSETATTR_FLUSH:
            ...
            // return error as console_device.h implements some attribute
            // changes
            return 0;
        default:
            return -ENOTTY;  // Means the operation does not apply to this
                             // descriptor
    }
}
...
#endif  // SERIAL_3_DMA
#if !defined(STM32F072xB)
                NVIC_SetPriority(USART3_IRQn, 15);  // Lowest priority for
                                                    // serial
                NVIC_EnableIRQ(USART3_IRQn);
#else   // STM32F072xB
                NVIC_SetPriority(USART3_4_IRQn, 15);
...
int STM32Serial::IRQdmaReadStop()
{
    ...
    defined(_ARCH_CORTEXM4_STM32L4)
    dmaRx->CCR                          = 0;
    static const unsigned int irqMask[] = {DMA_IFCR_CGIF5, DMA_IFCR_CGIF6,
                                           DMA_IFCR_CGIF3};
    ...
    static const unsigned int irqMask[] = {
        (DMA_HIFCR_CTCIF5 | DMA_HIFCR_CHTIF5 | DMA_HIFCR_CTEIF5 |
    ...
    static volatile unsigned long *const irqRegs[] = {
        &DMA2->HIFCR, &DMA1->HIFCR, &DMA1->LIFCR};
